...
export function BidForm() {
  ...
  return (
    <form className="bid-form" onSubmit={submit} aria-busy={status.type === "submitting"}>
      ...
      <label className="file-field"><span>Plans or scope</span><input name="attachments" type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.doc,.docx,.xls,.xlsx,.dwg,.dxf,.zip" /><small>Up to five files and 3 MB total. Include a plan-room link above for larger sets.</small></label>
      ...
      {status.type === "error" ? <p className="form-status is-error" role="alert">{status.message} Email estimating directly if the issue continues.</p> : null}
    </form>
  );
}
