...
test("homepage presents the JZ operating group with real field proof", async ({ page }) => {
  ...
  await expect(page.locator(".compact-hero-media")).toHaveJSProperty("playbackRate", narrow ? 0.64 : 1);
...
test("hero tells four phases without an oversized scroll gap", async ({ page }, testInfo) => {
  ...
  expect(heroHeight).toBeGreaterThanOrEqual(viewportHeight * 2.855);
  expect(heroHeight).toBeLessThanOrEqual(viewportHeight * 2.895);
